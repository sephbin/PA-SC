var displayGeometry = {{instance.displayGeom|safe}};

var Viewport = (function (THREE$1) {
	'use strict';
	...
	function Cameras(width, height) {
		...
		this._width = width;
		this._height = height;
		this.perspCamera = new THREE$1.PerspectiveCamera(30, this._width/this._height, 0.1, 1000);

		this.perspCamera.position.set( 20, 20, 20 );
		this.perspCamera.up = new THREE$1.Vector3(0,0,1);
		this.perspCamera.lookAt(0,0,0);

		var flashlight = new THREE$1.DirectionalLight( 0xff0000, 1 );
	...
	function AxisHelper(size) {
		var axesHelper = new THREE$1.AxesHelper(size);
	...
	function GridHelper(size, divisions) {
		var gridHelper = new THREE$1.GridHelper(size, divisions);
	...
	function Lights() {
		this.spotLight = new THREE$1.SpotLight( 0xffffff );
	...
	function sphere(data) {
		...
		if (!data.radius) {
			throw 'Sphere is missing radius'
		}
	...
	var solidPrimitives = /*#__PURE__*/Object.freeze({
		sphere: sphere,
		block: block
	});
	...
	*/
	...
	function Renderer(domParent, width, height) {
		...
		this._scene.add( new THREE$1.AmbientLight( 0xffffff ) );
		this._lights = new Lights();
		this._scene.add(this._lights);

		this.geometry = new THREE$1.BoxGeometry( 5, 5, 5 );
		this.material = new THREE$1.MeshPhongMaterial({ 
			color: 0x006DAA,
			shininess: 100,
			side: THREE$1.DoubleSide,
			clipShadows: true
		 });
		this.cube = new THREE$1.Mesh( this.geometry, this.material );
		// this._scene.add(this.cube);

		this.geo = new THREE$1.EdgesGeometry(this.geometry);
		this.mat = new THREE$1.LineBasicMaterial({ color: 0xFFFFFF, linewidth: 3 });
		this.wireframe = new THREE$1.LineSegments(this.geo, this.mat);
		// this._scene.add(this.wireframe);

		

		// Load external models
		this.loader = new THREE$1.ObjectLoader();

		/*
			Fetch external JSON data from a weblink. Uses promises to make sure data is obtained before adding to scene
		*/
		...
		//   let response = await fetch('https://gist.githubusercontent.com/emilyyleung/40af5cc73f438278aef92f8827d1f71b/raw/10189d5bdbd4bb95143d5985f4b4d9138c4e4db3/geojson.json')
		...
		function loadRedbackJson(loader, geojson, object) {
			var model = loader.parse(geojson, function(geometry) {
			...
			var color = decToRGB(geojson.materials[0].color)
			console.log(color)
			...
			for(var i = 0; i < model.children.length; i++) {
				console.log(model.children[i])
				for(var z = 0; z < geojson.materials.length; z ++) {
					console.log(geojson.materials[z])
					// var color = geojson.materials[z]
					// return colour;
				}
				// const mesh = new THREE.Mesh(model.children[i].geometry, material)
			...
			// const mesh = new THREE.Mesh(object.children, material);
			// return mesh;

			return object;
		};

		this._scene.add(loadRedbackJson(this.loader, displayGeometry, this.object));
		...
		this._controls = new THREE$1.OrbitControls(this._perspCamera, this.renderer.domElement);
	...
	Renderer.prototype.setClearColor = function(color, alpha) {
		this._clearColor = new THREE$1.Color(color);
  ...
  `);
	...
	SceneBuilder.prototype.validate = function(data) {
	...
	/*
		Obtain input value and check if it's a JSON object
		If it is, parse the JSON object and run the setGeometryEntity 
		function that validates the JSON

		@function setGeometryJson
		@param { Object } data The data to input into the Viewport
		@throws an error if unsupported datatype is found
		@return { function } setGeometryEntity which 
	*/
	...
	/*
		Takes the parsed JSON Object and validates it against the JSON Schema

		@function setGeometryEntity
		@param { Object } data The parsed JSON object
	*/

	Viewport.prototype.setGeometryEntity = function(data) {
		this._sceneBuilder.validate(data);
	};

	/*
		Creating a key press shortcut for saving (#futureWork)
	*/
